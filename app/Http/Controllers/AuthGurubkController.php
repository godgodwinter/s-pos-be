...
class AuthGurubkController extends Controller
{
    protected $me;
    ...
    public function __construct()
    {
        ...
        $this->middleware('auth:gurubk', ['except' => ['login', 'register']]);
    }
    ...
    public function me()
    {
        ...
        $getData = images::where('prefix', 'sekolah')->where('parrent_id', $item->sekolah_id);
        if ($getData->count() > 0) {
            $data = $getData->first();
            $listPhoto->logosekolah = URL::to($data->photo);
        }

        $getData = images::where('prefix', 'kepalasekolah')->where('parrent_id', $item->sekolah_id);
        if ($getData->count() > 0) {
            $data = $getData->first();
            $listPhoto->kepalasekolah = URL::to($data->photo);
        }

        $getData = images::where('prefix', 'gurubk')->where('parrent_id', $item->id);
        if ($getData->count() > 0) {
            $data = $getData->first();
    ...
    public function myprofileupdate(Request $request)
    {
        ...
        $sekolah = sekolah::find($item->sekolah_id);
        $sekolah->update($data);
    ...
    public function uploadLogoSekolah(Request $request)
    {
        ...
        // upload photo
        $path = 'uploads/images/sekolah';
        $file = $request->file('file');
        // $nama_file = rand() . $file->getClientOriginalName();
        $ext = $file->getClientOriginalExtension();
        $nama_file = $item->sekolah_id . '.' . $ext;
        ...
        if ($getData->count() > 0) {
            ...
            $data['prefix'] = 'sekolah';
            $data['parrent_id'] = $item->sekolah_id;
            $data['nama'] = "Photo Sekolah";
            images::create($data);
        }
    ...
    public function uploadKepalaSekolah(Request $request)
    {
        $item = $this->me;
        // set validation
        $validator = Validator::make($request->all(), [
        ...
        // upload photo
        $path = 'uploads/images/kepalasekolah';
        $file = $request->file('file');
        // $nama_file = rand() . $file->getClientOriginalName();
        $ext = $file->getClientOriginalExtension();
        $nama_file = $item->sekolah_id . '.' . $ext;
        ...
        if ($getData->count() > 0) {
            ...
            $data['prefix'] = 'kepalasekolah';
            $data['parrent_id'] = $item->sekolah_id;
            $data['nama'] = "Photo kepalasekolah";
            images::create($data);
        }
    ...
    public function uploadGuruBk(Request $request)
    {
        $item = $this->me;
        // set validation
        $validator = Validator::make($request->all(), [

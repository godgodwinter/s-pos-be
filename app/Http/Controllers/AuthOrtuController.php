...
class AuthOrtuController extends Controller
{
    ...
    {
        $ortu = $this->guard()->user();
        $sis_id = $ortu ? $ortu->siswa_id : 0;
        $identitas = Siswa::with('kelas')->where('id', $sis_id)->first();
        $s_id = $identitas ? $identitas->sekolah_id : 0;
        ...
        $this->middleware('auth:ortu', ['except' => ['login', 'register']]);
    }
    ...
    public function me()
    {
        ...
        $items = [];
        $ortu = $this->guard()->user();
        $identitas = Siswa::with('kelas')->where('id', $ortu->siswa_id)->first();
